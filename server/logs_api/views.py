...
from .exceptions import LogDataInvalid, LogDoesNotExist, LogAlreadyExists
...
class LogDetail(APIView):
    ...
    def delete(self, request, id):
        ...
        try:
            ...
            log.delete()
            return Response(
                {"message": f"Log with id {id} deleted"}, status=status.HTTP_200_OK
            )
...
class AggregateLogs(APIView):
    def get(self, request):
        ...
        try:
            ...
            data["overview_data"] = getOverviewData(
                logs
            )  # add the overview data to the data object

            # part 2: Annual Data
            data["annual_data"] = getAnnualData(
                logs
            )  # add the annual data to the data object

            # part 3: Severity Data
            data["severity_data"] = getSeverityData(
                logs
            )  # add the severity data to the data object
